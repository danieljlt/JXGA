...
class GenomeProcessor
{
    ...
    float returnFitnessFromGenome(const std::array<float, 23> normalizedParams, const PerceptualFeatures targetFeatures);
    
    // Used for getting set of features for the target audio sample
    PerceptualFeatures returnFeaturesFromGenome(const std::array<float, 23> normalizedParams);
