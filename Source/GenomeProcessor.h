...
class GenomeProcessor
{
    ...
    float returnFitnessFromGenome(const float normalizedParams[23], const PerceptualFeatures targetFeatures);
    
    // Used for getting set of features for the target audio sample
    PerceptualFeatures returnFeaturesFromGenome(const float normalizedParams[23]);
